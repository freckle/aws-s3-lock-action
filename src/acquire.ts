...
async function run() {
  try {
    const {name, bucket, expires, timeout, timeoutPoll, context} = getInputs()
    ...
    while (true) {
      ...
      if (result.tag === 'acquired') {
        ...
        core.info(`Lock ${color.bold(name)} ${color.green('acquired')} at ${keyDetails}`)
      ...
      if (timer.expired()) {
        core.error(`Lock ${color.bold(name)} ${color.red('already held')} by ${keyDetails}`)
        throw new Error('Lock was not acquired within timeout')
      }

      core.info(`Lock ${color.bold(name)} ${color.yellow('already held')} by ${keyDetails}`)
