...
export type AcquireLockResult =
  {tag: 'acquired'; acquiredKey: string} | {tag: 'not-acquired'; blockingKey: string}
...
export class S3Lock {
  ...
  async acquireLock(body: string): Promise<AcquireLockResult> {
    ...
    const keys = mapMaybe(output.Contents || [], o => validateObjectKey(this.prefix, o)).sort()
  ...
  async objectKeyDetails(key: string): Promise<string> {
    ...
    const contextLines = context && context === '' ? [] : [`Context: ${color.gray(context)}`]
